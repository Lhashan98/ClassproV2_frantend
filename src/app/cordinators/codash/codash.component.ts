...
@Component({
  selector: "app-codash",
  templateUrl: "./codash.component.html",
  styleUrls: ["./codash.component.css"],
})
export class CodashComponent implements OnInit {
  classArray: any[] = [];
  ...
  getAllclass() {
    this.http
      .get("http://localhost:8002/addclass/getAll")
  ...
  getAllCourse() {
    this.http
      .get(
        `http://localhost:8002/addcourse/getAll?department=${this.department}`
      )
      .subscribe((resultData: any) => {
        console.log(resultData);
  ...
  filterClassroomsByBuilding(buildingName: string): void {
    ...
    if (buildingName) {
      this.filteredClassrooms = this.filteredClassroomsByType.filter(
    ...
    console.log(this.filteredClassrooms)
    this.filteredClassroomsByBuil=this.filteredClassrooms;
  }
  ...
  async filterclassroomsByTime() {
    const filteredClassroomsByTime: any[] = [];
    this.filteredClassroomsForAvailable = [];
    if (!this.requestdate || !this.starttime || !this.endtime) {
      return;
    }
    for (const classroom of this.filteredClassroomsByBuil) {
      await this.addReportService
        .getReportsByClassId({
        ...
        .subscribe((res: any) => {
          if (res.date?.length == 0) {
            filteredClassroomsByTime.push(classroom);
          }
        });
    }
